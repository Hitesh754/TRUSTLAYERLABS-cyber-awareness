    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="border-b border-slate-800 bg-slate-950/90 backdrop-blur-md sticky top-0 z-40">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <span className="text-xs text-slate-400">{t(`scams.${scam.id}.name`, scam.name)}</span>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
        ...
        {/* {t('scamLibrary.commonIndicators', 'Common Indicators')} */}
        <div className="rounded-2xl p-6 bg-slate-900 border border-slate-800">
          <div className="flex items-center gap-2 mb-4">
            <AlertTriangle className="w-5 h-5 text-amber-500" />
            <h2 className="text-lg font-bold text-white">{t('scamLibrary.commonIndicators', 'Common Indicators')}</h2>
          </div>
        ...
        {/* {t('scamLibrary.preventionTips', 'Prevention Tips')} */}
        <div className="rounded-2xl p-6 bg-slate-900 border border-slate-800">
          <div className="flex items-center gap-2 mb-4">
            <Shield className="w-5 h-5 text-cyan-500" />
            <h2 className="text-lg font-bold text-white">{t('scamLibrary.preventionTips', 'Prevention Tips')}</h2>
          </div>
          <ul className="space-y-3">
        ...
        {scam.examples && scam.examples.length > 0 && (
          <div className="rounded-2xl p-6 bg-slate-900 border border-slate-800">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-lg font-bold text-white">{t('scamLibrary.realWorldExamples', 'Real-World Examples')}</h2>
            </div>
            <div className="space-y-3">
              {scam.examples.map((example, index) => (
                <div key={index} className="p-4 rounded-lg bg-slate-800 border border-slate-700">
                  <p className="text-sm text-slate-300 italic">"{t(`scams.${scam.id}.examples.${index}`, example)}"</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-5"
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">{t('scamLibrary.victimDesc', 'Report immediately to help protect others.')}</p>
          </div>
