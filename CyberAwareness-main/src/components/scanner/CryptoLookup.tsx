      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-12 gap-4">
          ...
          <div className="text-center">
            ...
            <p className="text-[11px] text-slate-600 mt-1 uppercase tracking-widest font-semibold">{t('cryptoLookup.traversingLedger')}</p>
          </div>
        </div>
      )}
        ...
        {result && !loading && (
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5 bg-slate-800/40 border border-slate-800 rounded-xl">
              <div>
                <p className="text-slate-400 text-xs uppercase tracking-wider">{result.network} {t('cryptoLookup.publicKey')}</p>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="p-4 bg-slate-900 border border-slate-850 rounded-xl">
                <p className="text-slate-400 text-[10px] uppercase font-bold tracking-wider">{t('cryptoLookup.currentBalance')}</p>
                <p className="text-sm font-mono font-bold text-white mt-1.5">{result.balance}</p>
              </div>

              <div className="p-4 bg-slate-900 border border-slate-850 rounded-xl">
                <p className="text-slate-400 text-[10px] uppercase font-bold tracking-wider">{t('cryptoLookup.txCount')}</p>
                <p className="text-sm font-mono font-bold text-white mt-1.5">{result.txCount}</p>
              </div>

              <div className="p-4 bg-slate-900 border border-slate-850 rounded-xl">
                <p className="text-slate-400 text-[10px] uppercase font-bold tracking-wider">{t('cryptoLookup.totalReceived')}</p>
                <p className="text-sm font-mono font-bold text-emerald-400 mt-1.5">{result.totalReceived}</p>
              </div>

              <div className="p-4 bg-slate-900 border border-slate-850 rounded-xl">
                <p className="text-slate-400 text-[10px] uppercase font-bold tracking-wider">{t('cryptoLookup.totalSent')}</p>
            ...
            {result.flaggedEntities.length > 0 && (
              <div className="p-5 bg-red-950/15 border border-red-900/30 rounded-xl space-y-3">
                <h4 className="text-xs text-red-300 font-bold uppercase tracking-wider">{t('cryptoLookup.threatFlags')}</h4>
            ...
            {result.txHistory.length > 0 && (
              <div className="p-5 bg-slate-900 border border-slate-850 rounded-xl space-y-4">
                <div className="flex items-center gap-2 border-b border-slate-800 pb-2.5">
                  ...
                  <h3 className="text-sm font-semibold text-white">{t('cryptoLookup.recentTransactions')}</h3>
                </div>
                    ...
                    <div key={idx} className="p-3 bg-slate-950/40 border border-slate-850 rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-slate-500 font-mono">{t('cryptoLookup.partner')}: {tx.partner}</p>
                        </div>
                      </div>
            ...
            <div className="p-5 bg-gradient-to-t from-slate-950 to-slate-900 border border-slate-850 rounded-xl">
              <h4 className="text-sm font-semibold text-white mb-3">{t('cryptoLookup.securityActions')}</h4>
