      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-12 gap-4">
          ...
          <div className="text-center">
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-600 mt-1 uppercase tracking-widest font-semibold">Running data registry query...</p>
          </div>
        </div>
      )}
        ...
        {result && !loading && (
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5 bg-white dark:bg-slate-800/40 border border-gray-200 dark:border-slate-800 rounded-xl shadow-sm">
              <div>
                <p className="text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider">Target Mail Address</p>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="p-4 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-850 rounded-xl shadow-sm animate-fade-in">
                <p className="text-slate-550 dark:text-slate-400 text-[10px] uppercase font-bold tracking-wider">SMTP Server</p>
                <p className="text-sm font-semibold text-slate-850 dark:text-white mt-1.5">
                  {result.smtpCheck === 'ACTIVE' ? (
                    <span className="text-emerald-600 dark:text-emerald-400 font-medium">✓ Deliverable</span>
                  ) : (
                    <span className="text-red-650 dark:text-red-400 font-bold">✗ Dead/Invalid</span>
                  )}
                </p>
              </div>

              <div className="p-4 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-850 rounded-xl shadow-sm animate-fade-in">
                <p className="text-slate-550 dark:text-slate-400 text-[10px] uppercase font-bold tracking-wider">MX Records</p>
                <p className="text-sm font-semibold text-slate-850 dark:text-white mt-1.5">
                  {result.mxRecordsFound ? (
                    <span className="text-emerald-600 dark:text-emerald-400 font-medium">✓ Resolved</span>
                  ) : (
                    <span className="text-red-650 dark:text-red-400 font-bold">✗ None</span>
                  )}
                </p>
              </div>

              <div className="p-4 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-850 rounded-xl shadow-sm animate-fade-in">
                <p className="text-slate-550 dark:text-slate-400 text-[10px] uppercase font-bold tracking-wider">Temp Mailbox</p>
                <p className="text-sm font-semibold text-slate-850 dark:text-white mt-1.5">
                  {result.isDisposable ? (
                    <span className="text-red-650 dark:text-red-400 font-bold">Yes</span>
                  ) : (
              ...
              <div className="p-4 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-850 rounded-xl shadow-sm animate-fade-in">
                <p className="text-slate-550 dark:text-slate-400 text-[10px] uppercase font-bold tracking-wider">Data Leaks</p>
                <p className="text-sm font-semibold text-slate-850 dark:text-white mt-1.5">
            ...
            {result.breaches.length > 0 ? (
              <div className="p-5 bg-red-50/50 dark:bg-red-950/10 border border-red-200 dark:border-red-900/30 rounded-xl space-y-4">
                <div className="flex items-center gap-2.5">
                  ...
                  <h3 className="text-sm font-semibold text-red-800 dark:text-red-300">Data Leak Registry Records</h3>
                </div>
                ...
                <div className="space-y-4">
                  {result.breaches.map((breach, index) => (
                    <div key={index} className="p-4 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-850 rounded-lg shadow-sm">
                      ...
                      <div className="mt-3">
                        <p className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold tracking-wider mb-1.5">Exposed parameters</p>
              ...
              <div className="p-5 bg-emerald-50/50 dark:bg-emerald-950/10 border border-emerald-200 dark:border-emerald-900/30 rounded-xl flex gap-3.5 items-center">
                ...
                <div>
                  <h3 className="text-sm font-bold text-emerald-800 dark:text-emerald-300">No public leaks discovered</h3>
                  <p className="text-slate-600 dark:text-slate-400 text-xs mt-0.5">This email address was not found in known public cyber breach datasets.</p>
                </div>
              </div>
            )}
