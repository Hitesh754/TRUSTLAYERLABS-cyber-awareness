  ...
  return (
    <section id="top" className="ph-hero">
      ...
      <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} className="ph-shell ph-hero-inner">
        <div className="ph-kicker"><Radar size={16} /> {t('phishingSimulatorTag')}</div>
        <h1>{t('phishingSimulator')}</h1>
        <p className="ph-typing">{text}<span /></p>
        <p className="ph-hero-copy">{t('phishingSimulatorDesc')}</p>
        <div className="ph-actions">
          <a className="ph-btn ph-btn-primary" href="#simulator"><Play size={18} /> {t('awareness.startLearning')}</a>
          <a className="ph-btn ph-btn-ghost" href="#education">{t('common.learnMore')} <ArrowRight size={18} /></a>
        </div>
      </motion.div>
    </section>
  );
